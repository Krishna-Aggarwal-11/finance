...
export const ImportCard = ({
    ...
    const handleContinue = () =>{
        ...
        const formattedData = arrayOfData.map((item) => ({
            ...
            date : format(parse(item.date , dataFormat, new Date()) , ouptutFormat)
        }));
        ...
        <Card className="border-none drop-shadow-sm">
          <CardHeader className=" gap-y-2 lg:flex-row lg:justify-between lg:items-center">
            ...
            <div className="flex items-center flex-col lg:flex-row gap-y-2 gap-x-2">
              <Button onClick={onCancel} className="w-full lg:auto" size={"sm"}>
                
                Cancel
              </Button>
              <Button className="w-full lg:auto" size={"sm"} disabled = {progress < requiredOptions.length}
